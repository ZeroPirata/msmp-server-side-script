BlockEvents.rightClicked((event) => {
  ...
  if (!server.persistentData.contains(playerKey)) {
    // Se a permissão foi dada (ou não era um baú registrado), mas não há dados de recompensa,
    // apenas permite que o baú abra normalmente e sai.
    if (isRegisteredChest && hasPermission) {
      console.log(`[MSMP] ${player.name.getString()} abriu seu baú com sucesso (Sem Recompensas Pendentes)`);
    }
    return;
  }
  ...
  if (data.used) {
    // Evita a abertura se o loot já foi pego, mas permite fechar/abrir para conferir o loot nativo do lootr.
    // Se você quer BARRAR completamente, descomente as próximas duas linhas.
  ...
  if (lootMultiplier > 1.0) {
    ...
    player.tell(`§6§l+${((lootMultiplier - 1.0) * 100).toFixed(0)}% Loot Extra (Escalonamento por Jogador!)`);
  }
